...
describe("SequenceDomain", () => {
  ...
  it("fails when length mismatches", () => {
    const seq = new SequenceDomain<[number]>([new FiniteDomain([1])]);
    expect(seq.contains([])).toBe(false);
  });

  it("fails for non-array input", () => {
    const seq = new SequenceDomain<[number]>([new FiniteDomain([1])]);
    // Testing defensive behavior - domain.contains should handle non-arrays
    expect(seq.contains("not-array")).toBe(false);
  });
});
