...
describe('FiniteDomainGenerator', () => {
  it('should generate values from a finite domain', () => {
    ...
    const domain: FiniteDomain<boolean> = new FiniteDomain([true, false] as const);
    const rng: RNG = {
      random: () => 0.3,
    };
  ...
  it('should select last value when RNG returns value close to 1', () => {
    ...
    const domain: FiniteDomain<number> = new FiniteDomain([1, 2, 3, 4, 5] as const);
    const rng: RNG = {
      random: () => 0.99,
    };
  ...
  it('should generate values from single-element domain', () => {
    const generator = new FiniteDomainGenerator<string>();
    const domain: FiniteDomain<string> = new FiniteDomain(['only'] as const);
    const rng: RNG = {
      random: () => Math.random(),
    };
  ...
  it('should use RNG to determine selection', () => {
    const generator = new FiniteDomainGenerator<number>();
    const domain: FiniteDomain<number> = new FiniteDomain([10, 20, 30] as const);
  ...
  it('should handle large finite domains', () => {
    ...
    const domain: FiniteDomain<number> = new FiniteDomain(values as readonly number[]);
    const rng: RNG = {
      random: () => Math.random(),
    };
